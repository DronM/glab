...
func (pm *ItemSearch_Controller_find_items) Run(app gobizap.Applicationer, serv srv.Server, sock socket.ClientSocketer, resp *response.Response, rfltArgs reflect.Value) error {
	...
	if args.Criteria.Item_feature_qualities != "" {
		cond_cnt++
		cond_list = append(cond_list, fmt.Sprintf("mnf_b.quality_level = $%d", cond_cnt))
	...
	cond_list = append(cond_list, fmt.Sprintf("fld.item_priority_id IN (%s)", s)) //always!!!
	...
	if args.Criteria.Item_feature_options != nil && len(args.Criteria.Item_feature_options) > 0 {		
		var incl_opts strings.Builder
		for i, opt := range args.Criteria.Item_feature_options {
			if i > 0 {
				incl_opts.WriteString(",")
			}
			incl_opts.WriteString(fmt.Sprintf("%d", opt.Id))
			//+condition
			pref := "" //нужно проверить, чтобы опция обязательно была
			if !opt.Val {
				pref = "NOT " //нужно проверить, чтобы опции обязательно не было
			}
			cond_list = append(cond_list, fmt.Sprintf(pref + `opts.vals @> '[{"item_feature_id": %d}]'`, opt.Id))
		}
		opt_join = fmt.Sprintf(`LEFT JOIN (
			SELECT
				t.item_id,
				jsonb_agg(
					jsonb_build_object(
						'item_feature_id', t.item_feature_id,
						'val', t.val
					)
				) AS vals
			FROM item_feature_vals AS t
			WHERE t.item_feature_id IN (%s)
			GROUP BY t.item_id
		) AS opts ON opts.item_id = it.id`, incl_opts.String())
	}
	...
	query := fmt.Sprintf(`SELECT
		...
		FROM (
			...
			WHERE v2.supplier_item_id = s_it.id)
		) AS sub
