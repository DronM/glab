...
func (pm *Attachment_Controller_get_object) Run(app gobizap.Applicationer, serv srv.Server, sock socket.ClientSocketer, resp *response.Response, rfltArgs reflect.Value) error {
	return gobizap.GetObjectOnArgs(app, resp, rfltArgs, app.GetMD().Models["AttachmentList"], &models.AttachmentList{}, sock.GetPresetFilter("AttachmentList"))	
}
...
func (pm *Attachment_Controller_get_list) Run(app gobizap.Applicationer, serv srv.Server, sock socket.ClientSocketer, resp *response.Response, rfltArgs reflect.Value) error {
	return gobizap.GetListOnArgs(app, resp, rfltArgs, app.GetMD().Models["AttachmentList"], &models.AttachmentList{}, sock.GetPresetFilter("AttachmentList"))	
}
...
func (pm *Attachment_Controller_delete_file) Run(app gobizap.Applicationer, serv srv.Server, sock socket.ClientSocketer, resp *response.Response, rfltArgs reflect.Value) error {
	...
	args := rfltArgs.Interface().(*models.Attachment_delete_file)
	...
	if _, err := conn.Exec(context.Background(),
		...
		return gobizap.NewPublicMethodError(response.RESP_ER_INTERNAL, fmt.Sprintf("Attachment_Controller_delete_file conn.Exec(): %v",err))	
	}
	...
	if view.FileExists(att_n) {
		if err := os.Remove(att_n); err != nil {
			return gobizap.NewPublicMethodError(response.RESP_ER_INTERNAL, fmt.Sprintf("Attachment_Controller_delete_file os.Remove(): %v",err))	
		}
	}
...
func (pm *Attachment_Controller_get_file) Run(app gobizap.Applicationer, serv srv.Server, sock socket.ClientSocketer, resp *response.Response, rfltArgs reflect.Value) error {
	...
	args := rfltArgs.Interface().(*models.Attachment_get_file)
...
func (pm *Attachment_Controller_add_file) Run(app gobizap.Applicationer, serv srv.Server, sock socket.ClientSocketer, resp *response.Response, rfltArgs reflect.Value) error {
	...
	args := rfltArgs.Interface().(*models.Attachment_add_file)	
...
func RemoveAttachment(conn *pgx.Conn, baseDir string, ref models.Ref_Type, contentID string) error {
	if _, err := conn.Exec(context.Background(),
		`DELETE FROM attachments
		WHERE ref->>'dataType' = $1
			AND (ref->'keys'->>'id')::int = $2
	...
	att_n := getAttachmentCacheFileName(baseDir, contentID)
	if view.FileExists(att_n) {
		if err := os.Remove(att_n); err != nil {
			return gobizap.NewPublicMethodError(response.RESP_ER_INTERNAL, fmt.Sprintf("DeleteAttachment os.Remove(): %v",err))	
		}
	}
	preview_fn := getPreviewCacheFileName(baseDir, contentID)
	if view.FileExists(preview_fn) {
		if err := os.Remove(preview_fn); err != nil {
			return gobizap.NewPublicMethodError(response.RESP_ER_INTERNAL, fmt.Sprintf("DeleteAttachment os.Remove(): %v",err))	
		}
