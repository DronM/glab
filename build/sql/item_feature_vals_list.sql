-- Function: item_feature_vals_list(in_item_id int, in_item_folder_id int)

-- DROP FUNCTION item_feature_vals_list(in_item_id int, in_item_folder_id int) CASCADE;


CREATE OR REPLACE FUNCTION item_feature_vals_list(in_item_id int, in_item_folder_id int)
  RETURNS TABLE (
  	features json
  ) AS
$$
...
FROM item_folder_feature_groups AS f_features
LEFT JOIN item_feature_groups AS gr ON gr.id= f_features.item_feature_group_id
...
where f_features.item_folder_id=17
*/
...
SELECT
	json_build_object(
		...
		'val', (SELECT val FROM item_feature_vals WHERE item_id = in_item_id AND item_feature_id = f.id)
	)
FROM
(
	SELECT
...
$$
  ...
  COST 100;
ALTER FUNCTION item_feature_vals_list(in_item_id int, in_item_folder_id int) OWNER TO ;
