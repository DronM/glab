...
<xsl:template name="initApp">
	...
	<xsl:choose>
	<xsl:when test="/document/model[@id='ServerVars']/row/role_id='person'">
...
<xsl:template name="page_header">
	...
	<div class="navbar navbar-inverse">
		...
		<div class="navbar-header">
			<a class="navbar-brand" href="/">Паспорт обученности
			</a>
		</div>
		</xsl:when>
		<xsl:otherwise>
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<li class="dropdown dropdown-user">
					...
					<ul class="dropdown-menu dropdown-menu-right">
						...
						<xsl:if test="not(/document/model[@id='ServerVars']/row/role_id='person')">
						<li class="divider"></li>
