...
<xsl:template name="initHead">
	...
	<title>Лаборатория Автостекла</title>
</xsl:template>
...
<!-- ERROR 
<xsl:template match="model[@id='Response']/row/result &lt;&gt;'0'">
...
<xsl:template name="checkForError">
	...
	<xsl:when test="not($er_num='0') and not(/document/model[@id='ServerVars']/row/role_id='')">		
...
<xsl:template name="page_header">
	...
	<div class="container-fluid">
		...
		<xsl:choose>
		<xsl:when test="/document/model[@id='ServerVars']/row/role_id=''">
			...
			<ul class="nav navbar-nav navbar-right">			
				...
				<li class="dropdown dropdown-user">
					<a class="dropdown-toggle" data-toggle="dropdown">
						<xsl:variable name="tm_photo" select="/document/model[@id='ServerVars']/row/tm_photo" />
